...
export type EquipmentQuotationDecisionMode =
  | 'approve'
  | 'reject'
  | 'request_changes'

export interface EquipmentQuotationDecisionValues {
...
interface EquipmentQuotationApprovalDialogProps {
  open: boolean
  mode: EquipmentQuotationDecisionMode
  quoteCode: string
  isLoading?: boolean
  onClose: () => void
  onSubmit: (values: EquipmentQuotationDecisionValues) => Promise<void> | void
}

const buildTodayValue = () => new Date().toISOString().slice(0, 10)

const createInitialValues = (): EquipmentQuotationDecisionValues => ({
  approvalChannel: '',
  approvalReference: '',
...
const EquipmentQuotationApprovalDialog = ({
  open,
  mode,
  quoteCode,
  isLoading = false,
  onClose,
  onSubmit
}: EquipmentQuotationApprovalDialogProps) => {
  const [values, setValues] = useState<EquipmentQuotationDecisionValues>(
    createInitialValues
  )
      ...
      <DialogContent dividers>
        <Stack spacing={2}>
          <Alert severity={isRejectMode ? 'warning' : 'success'}>
            {isRejectMode
              ? `Esta acción registra formalmente la respuesta del cliente para ${quoteCode}.`
              : isRequestChangesMode
                ? `Esta acción deja trazada la solicitud de modificación del cliente para ${quoteCode}.`
                : `Esta acción registra formalmente la aprobación del cliente para ${quoteCode}.`}
          </Alert>
...
export default EquipmentQuotationApprovalDialog
