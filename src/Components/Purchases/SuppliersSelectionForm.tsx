  ...
  useEffect(() => {
    const loadCriteria = async () => {
      try {
        ...
        setError('Error cargando criterios de evaluación')
      }
    }
    loadCriteria()
  }, [])
              ...
              <Paper key={category} sx={{ p: 2, mb: 3 }}>
                ...
                <FormGroup>
                  {(groupedCriteria[category] || [])
                    .filter(
                      (c) =>
                        c.type === 'Both' || c.type === formState.providerType
                    )
                    .map((criterion) => {
                      const isSelected = (
                      ...
                      return (
                        <Box key={criterion.id} sx={{ mb: 1 }}>
                          {[
                            ...
                            <RadioGroup
                              value={formState.selections[category]?.[0] || ''}
                              onChange={(e) =>
                ...
                <Typography sx={{ mt: 2 }}>
                  Puntuación {category}: {formState.scores[category] || 0} pts
                </Typography>
              </Paper>
            ))}
